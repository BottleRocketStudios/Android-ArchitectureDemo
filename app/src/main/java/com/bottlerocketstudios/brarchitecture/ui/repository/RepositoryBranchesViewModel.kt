...
class RepositoryBranchesViewModel : BaseViewModel() {
    ...
    // region Init
    init {
        launchIO { currentRepoName.collect { repoName -> getRepoBranches(repoName) } }
    }
    // endregion
    ...
    val itemCount: StateFlow<Int> = srcBranches.map { it.size }.groundState(0)
    val uiModels: StateFlow<List<RepositoryBranchItemUiModel>> =
            srcBranches
                    .map { branches ->
                        branches.map { branch ->
                            RepositoryBranchItemUiModel(
                                    ...
                                    status =
                                            StringIdHelper.Id(
                                                    R.string.home_branches_status_placeholder
                                            )
                            )
                        }
                    }
