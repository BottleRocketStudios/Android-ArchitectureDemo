...
/**
 * Ktor based implementation of the Bitbucket API. Only a subset of endpoints are implemented for
 * demonstration purposes. Additional endpoints can be added following the same pattern.
 */
internal class BitbucketServiceKtor : KoinComponent {
    ...
    suspend fun getRepositorySource(
            workspace: String,
            repo: String
    ): BitbucketPagedResponse<List<RepoFileDto>> =
    ...
    suspend fun getPullRequests(
            selectedUser: String
    ): BitbucketPagedResponse<List<PullRequestDto>> =
    ...
    suspend fun getPullRequestsWithQuery(
            ...
            client
                    .get("2.0/pullrequests/$selectedUser") {
                        url { parameters.append("state", state) }
                    }
                    .body()

    suspend fun getSnippets(): BitbucketPagedResponse<List<SnippetDto>> =
            client.get("2.0/snippets") { url { parameters.append("role", "owner") } }.body()

    suspend fun deleteSnippet(
            workspaceId: String,
            encodedId: String
    ): io.ktor.client.statement.HttpResponse = client.delete("2.0/snippets/$workspaceId/$encodedId")
    ...
    suspend fun getWorkspaces(): BitbucketPagedResponse<List<WorkspaceDto>> =
            client.get("2.0/workspaces").body()
}
